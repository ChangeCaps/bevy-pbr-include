...
layout(set = 2, binding = 1) uniform ShadowLights {
    ShadowDirectionalLight shadow_directional_lights[MAX_DIRECTIONAL_LIGHTS];
};

layout(set = 2, binding = 2) uniform texture2DArray DirectionalLightTexture;
layout(set = 2, binding = 3) uniform sampler DirectionalLightSampler;
...
vec3 point_light(PointLight light, vec3 world_pos, float roughness, float NdotV, vec3 N, vec3 V, vec3 R, vec3 F0, vec3 diffuseColor) {
    vec3 light_to_frag = light.pos.xyz - world_pos;
...
Pbr default_pbr(vec3 camera_pos, vec3 world_pos, vec3 normal) {
    Pbr pbr;

    pbr.camera_pos = camera_pos;
    pbr.world_pos = world_pos;
    pbr.normal = normal;
    pbr.normal_map = vec3(0.0);
    pbr.tangent = vec4(0.0);
    pbr.albedo = vec3(1.0);
    pbr.roughness = 0.089;
    pbr.metallic = 0.01;
    pbr.occlusion = 1.0;
    pbr.reflectance = 0.5;
    pbr.emissive = vec4(0.0, 0.0, 0.0, 1.0);

    return pbr;
}

vec4 pbr(Pbr pbr) {
    vec4 output_color = vec4(pbr.albedo, 1.0);
    ...
    if (length(pbr.normal_map) > 0.0) {
        vec3 T = normalize(pbr.tangent.xyz);
        vec3 B = cross(N, T) * pbr.tangent.w;
        ...
        mat3 TBN = mat3(T, B, N);
        N = TBN * normalize(pbr.normal_map * 2.0 - 1.0);
    }

    float occlusion = pbr.occlusion;

    vec4 emissive = pbr.emissive;

    vec3 V = normalize(pbr.camera_pos - pbr.world_pos);
    // Neubelt and Pettineo 2013, "Crafting a Next-gen Material Pipeline for The Order: 1886"
    float NdotV = max(dot(N, V), 1e-4);

    // Remapping [0,1] reflectance to F0
    // See https://google.github.io/filament/Filament.html#materialsystem/parameterization/remapping
    vec3 F0 = 0.16 * pbr.reflectance * pbr.reflectance * (1.0 - metallic) + output_color.rgb * metallic;
    ...
    for (int i = 0; i < int(NumLights.x) && i < MAX_POINT_LIGHTS; ++i) {
        light_accum += point_light(PointLights[i], pbr.world_pos, roughness, NdotV, N, V, R, F0, diffuseColor);
    }
    for (int i = 0; i < int(NumLights.y) && i < MAX_DIRECTIONAL_LIGHTS; ++i) {
        ShadowDirectionalLight shadow_light = shadow_directional_lights[i];

        vec4 p = shadow_light.viewProj * vec4(pbr.world_pos, 1.0);
        vec2 uv = p.xy;
        uv.y *= -1.0;
        uv += 1.0;
        uv /= 2.0;
